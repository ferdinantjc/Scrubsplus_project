...
const Add = ({ token }) => {
  ...
  const [skinToneMatch, setSkinToneMatch] = useState(""); // ✅ NEW
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      formData.append("skinToneMatch", skinToneMatch); // ✅ NEW
      ...
      if (response.data.success) {
        ...
        setSizes([]);
        setBestseller(false);
        setSkinToneMatch(""); // ✅ Reset skin tone
      } else {
      ...
      <form onSubmit={onSubmitHandler} className='flex flex-col w-full items-start gap-4'>
        ...
        <div className='flex flex-col sm:flex-row gap-4 w-full'>
          ...
          <div>
            ...
            <input onChange={(e) => setPrice(e.target.value)} value={price} type="number" placeholder="25" className='px-3 py-2 border rounded w-[120px]' required />
          </div>
        </div>

        {/* ✅ Skin Tone Match */}
        <div>
          <p className='mb-2 font-medium'>Recommended for Glow types</p>
          <select
            onChange={(e) => setSkinToneMatch(e.target.value)}
            value={skinToneMatch}
            className='px-3 py-2 border rounded'
            required
          >
            <option value="">-- Select Glow Type --</option>
            <option value="fair">Fair Glow</option>
            <option value="medium">Warm Medium</option>
            <option value="dark">Deep Radiance</option>
          </select>
        </div>
